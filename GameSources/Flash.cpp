...
namespace basecross {
	// コンストラクタ
	Flash::Flash(const shared_ptr<Stage>& stage) :
		GameObject(stage),
	...
	void Flash::CreateSprite(const Vec3& position, const Vec2& size, const wstring& texKey)
	{
	...
	void Flash::UpdateFlashingSprite(const int flashSpeed)
	{
		auto& app = App::GetApp();
	...
	void Flash::UpdateFlashingSprite(const int flashSpeed, const int afterFlashSpeed, const bool speedChangeTrigger)
	{
		auto& app = App::GetApp();
		auto delta = app->GetElapsedTime();	// 時間の取得
	...
	void Flash::UpdateSeekSizeSprite(const Vec2& afterSize, const float seekSecond, const SeekType& seekType)
	{
		// ステートが停止状態でなければ
	...
	void Flash::UpdateSeekSizeSprite(const Vec2& afterSize, const float seekSecond, const SeekType& seekType, const float waitSecond)
	{
		// 時間の取得
		auto& app = App::GetApp();
		auto delta = app->GetElapsedTime();
	...
	void Flash::UpdateFadeSprite(const float fadeSecond)
	{
		// 時間の取得
		auto& app = App::GetApp();
		auto delta = app->GetElapsedTime();
