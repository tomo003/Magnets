...
namespace basecross {
	...
	void Player::OnCollisionEnter(shared_ptr<GameObject>& Other) {
		...
		if (ptrGearFloor) {
			...
			if (m_eMagPole == EState::eN && GesrFloorState == (int)EState::eMetal) {
				m_gravityComp->SetGravityZero();
	...
	void Player::OnCollisionExit(shared_ptr<GameObject>& Other) {
		...
		if (ptrGoal && m_pos.x > ptrGoal->GetComponent<Transform>()->GetPosition().x && !isStop)
		{
			...
			if (m_eMagPole == EState::eFalse) {
				m_ptrDraw->SetMeshResource(L"PlayerBrackSmile_MESH");//笑顔
			}
			else if (m_eMagPole == EState::eN) {
				m_ptrDraw->SetMeshResource(L"PlayerRedSmile_MESH");//笑顔
			}
			AnimationPlayer(FRONT);
			isStop = true;
		}
